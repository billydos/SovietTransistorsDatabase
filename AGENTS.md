...
1. **Валидация дублируется** в двух местах: `Domain/*Validator` (понятные сообщения по-русски) и CHECK-ограничения в `Data/SqliteDatabase.cs` (DDL). Меняя правило параметра или поля ratings, обновляй оба места плюс сообщения об ошибках.
...
- DML в `RelationalTransistorDatabase` — только переносимые конструкции (`@`-параметры, `COALESCE`, `LIMIT`); диалектозависимые вещи — только в `CreateTableSql`/`OpenConnection` переопределениях.
