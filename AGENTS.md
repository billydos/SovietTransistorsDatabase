...
1. **Валидация электрических параметров декларативна**: правила (границы, потолок значения, варианты условий) заданы в каталоге `Domain/ElectricalParameters.cs` (`ElectricalParameterCatalog`) и `Domain/ParameterConditions.cs` (именованные спеки `ConditionSpecs`). Из каталога генерируются: C#-валидатор, раздел параметров `help`, список полей секции jsonc и CHECK-ограничения `electrical_parameters` (`Data/ElectricalParametersDdl.CreateTableSql`, компонуемый в `Data/SqliteDatabase.cs`). Новый код параметра — правка каталога плюс SKILL.md. Валидация ratings и атрибутов по-прежнему дублируется в двух местах: `Domain/*Validator` (сообщения по-русски) и CHECK в `Data/SqliteDatabase.cs` — обновляй оба места. Исключения: `chk_letters` сознательно слабее домена (переносимого предиката «1–2 заглавные кириллические» в CHECK у SQLite/PostgreSQL/MariaDB нет — обоснование в комментарии DDL, не «усиливать»); список полей атрибутов имеет единый источник — `TransistorAttributes.FieldNames` (`Domain`), из него генерируются `chk_any_attribute` (`Data/AttributesDdl`) и `AttributeColumns` (спецификация чтения), новое свойство записи подхватывается автоматически (пин-тест Domain ловит рассинхронизацию).
...
- `.kilo/skills/soviet-transistors-database/SKILL.md` — регламент наполнения; синхронно обновлять при изменении формата jsonc (полный справочник полей и типичных ошибок там, дублировать в AGENTS.md не нужно).
